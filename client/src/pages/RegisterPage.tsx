	...
	const handleSubmit = async (e: React.FormEvent) => {
		...
		try {
			...
			toast({
				title: "Erreur SMS",
				description:
					"Le SMS n'a pas pu être envoyé, mais vous pouvez continuer",
				variant: "destructive",
			});
		} finally {
		...
		<div className="min-h-screen bg-white flex flex-col md:flex-row">
			<div className="hidden md:flex md:w-1/2 bg-secondary text-white p-12 flex-col justify-between">
				<div>
					...
					<p className="text-white/60 font-medium">
						Rejoignez la révolution logistique à Kinshasa.
					</p>
				</div>
				<div className="space-y-6">
					<div className="flex items-start gap-4">
						...
						<div>
							...
							<p className="text-white/60 text-sm">
								Recevez vos gains directement sur Mobile Money.
							</p>
						</div>
					</div>
					<div className="flex items-start gap-4">
					...
					<div className="flex items-start gap-4">
						...
						<div>
							...
							<p className="text-white/60 text-sm">
								Vos clients suivent leur colis en direct.
							</p>
						</div>
					</div>
				</div>
			...
			<div className="flex-1 p-8 md:p-20 overflow-y-auto">
				<div className="max-w-md mx-auto space-y-8">
					...
					<div className="grid grid-cols-2 gap-4">
						<div
							onClick={() => setSelectedRole("pro_seller")}
							className={`cursor-pointer p-5 rounded-2xl border-2 transition-all text-center ${
								role === "pro_seller"
						...
						>
							<Crown
								className={`h-8 w-8 mx-auto mb-2 ${
									role === "pro_seller" ? "text-primary" : "text-gray-400"
								}`}
							/>
						...
						>
							<Truck
								className={`h-8 w-8 mx-auto mb-2 ${
									role === "courier" ? "text-secondary" : "text-gray-400"
								}`}
							/>
					...
					<form onSubmit={handleSubmit} className="space-y-4">
						...
						{role === "pro_seller" && (
							<>
								<div className="space-y-2">
									...
									>
										<Store className="h-4 w-4 text-primary" /> Nom de la
										boutique *
									</Label>
									<Input
										id="shopName"
										name="shopName"
										required
										placeholder="Ex: Boutique Mama Jolie"
								...
								<div className="space-y-2">
									...
									>
										<div className="flex items-center space-x-2">
											<RadioGroupItem value="car" id="moto" />
						...
						<Button
							...
							className="w-full h-14 bg-secondary text-white font-bold rounded-xl text-lg mt-8"
							data-testid="button-register"
						>
							{isSubmitting ? (
								<>
									<Loader2 className="h-5 w-5 animate-spin mr-2" /> Envoi du
									code...
								</>
							) : (
								"S'inscrire"
			...
			<Dialog open={showOtpStep} onOpenChange={setShowOtpStep}>
				<DialogContent className="rounded-[2rem] p-8 max-w-sm">
					<DialogHeader>
						<DialogTitle className="text-center font-black text-2xl tracking-tight text-secondary">
							Vérification
						</DialogTitle>
					</DialogHeader>
					<div className="space-y-5 pt-4">
						...
						<div className="flex justify-center">
							...
							>
								<InputOTPGroup>
									<InputOTPSlot
										index={0}
										className="h-14 w-12 text-2xl font-black rounded-xl"
									/>
									<InputOTPSlot
										index={1}
										className="h-14 w-12 text-2xl font-black rounded-xl"
						...
						<Button
							...
							className="w-full h-14 bg-secondary hover:bg-secondary/90 text-white font-bold rounded-xl text-base"
							data-testid="button-verify-register-otp"
						>
							{isVerifying ? (
