...
import { Search, Truck, CheckCircle2, Clock, Package, ShieldCheck, ArrowLeft } from "lucide-react";
        ...
        <div className="flex gap-2">
          <div className="relative flex-1">
            ...
            <Input placeholder="ID de livraison" value={searchId} onChange={(e) => setSearchId(e.target.value)}
              className="pl-12 h-13 bg-white border-2 border-gray-100 rounded-2xl shadow-sm font-bold" data-testid="input-tracking" />
          </div>
          <Button onClick={() => handleSearch()} className="h-13 px-8 bg-primary text-secondary rounded-2xl shadow-lg font-black uppercase tracking-widest" data-testid="button-search">Rechercher</Button>
        </div>
        <AnimatePresence mode="wait">
          {foundDelivery ? (
            <motion.div key={foundDelivery.id} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }} className="space-y-8">
              <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
                <div>
                  <p className="text-sm text-gray-500 font-medium">Livraison #{foundDelivery.id.substring(0, 8)}</p>
                  <h2 className="text-3xl font-black text-secondary" data-testid="text-status-title">
                    {foundDelivery.status === 'delivered' ? 'Livré avec succès' : foundDelivery.status === 'in_transit' ? "En cours d'acheminement" : 'En préparation'}
                  </h2>
                </div>
                <Badge className={cn("px-6 py-2 rounded-full text-sm font-black uppercase shadow-lg",
                  foundDelivery.status === 'delivered' ? "bg-green-500 text-white" : foundDelivery.status === 'in_transit' ? "bg-blue-500 text-white" : "bg-amber-500 text-white"
                        ...
                        {steps.map((step) => {
                          ...
                          return (
                            <div key={step.status} className="relative flex items-center gap-6">
                              <div className={cn("z-10 w-12 h-12 rounded-2xl flex items-center justify-center shadow-lg",
                                isCompleted ? "bg-primary text-primary-foreground scale-110" : "bg-white text-slate-300 border-2 border-slate-100")}>
                                <Icon className="h-6 w-6" />
                              </div>
                              <p className={cn("text-sm font-black uppercase", isCompleted ? "text-secondary" : "text-slate-300")}>{step.label}</p>
                            </div>
                          );
                        })}
                      </div>
                  ...
                  <Card className="border-none shadow-xl bg-white rounded-[2rem] overflow-hidden">
                    <CardContent className="p-6 space-y-4">
                      <div>
                        ...
                        <h3 className="text-lg font-black text-secondary" data-testid="text-recipient">{foundDelivery.customerName}</h3>
            ...
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="text-center py-20 space-y-6 bg-gray-50 rounded-[3rem]">
              <div className="w-24 h-24 bg-white rounded-[2rem] flex items-center justify-center mx-auto shadow-sm border"><Package className="h-12 w-12 text-gray-300" /></div>
              <div className="space-y-2">
                <p className="text-2xl font-black text-secondary uppercase italic">Livraison introuvable</p>
                <p className="text-sm font-bold text-gray-400 uppercase tracking-widest">Vérifiez l'identifiant</p>
              </div>
            </motion.div>
          )}
        </AnimatePresence>
